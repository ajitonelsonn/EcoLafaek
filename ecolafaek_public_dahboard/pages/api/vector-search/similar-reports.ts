...
import { sendError, sendSuccess, validateRequired, validateReportId, validateLimit, validateThreshold } from './utils/api-response';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!validReportId) {
      return sendError(res, 400, 'Invalid report ID format', 'INVALID_REPORT_ID');
    }
    ...
    if (!Array.isArray(sourceRows) || sourceRows.length === 0) {
      return sendError(res, 404, 'Source report not found or has no embedding', 'REPORT_NOT_FOUND');
    }
    ...
    const [similarRows] = await connection.execute(`
      SELECT 
        ...
        w.name as waste_type,
        u.username,
        VEC_COSINE_DISTANCE(a.image_embedding, ?) as similarity_score
      FROM reports r
      JOIN analysis_results a ON r.report_id = a.report_id
      LEFT JOIN waste_types w ON a.waste_type_id = w.waste_type_id
      ...
      LIMIT ${searchLimit}
    `, [
      JSON.stringify(sourceReport.image_embedding),
      validReportId
    ]);
    ...
    `, [validReportId, sourceReport.latitude, sourceReport.longitude, sourceReport.latitude]);

    const similarReports = Array.isArray(similarRows) ? similarRows : [];
    const nearbyCount = Array.isArray(nearbyRows) && nearbyRows.length > 0 ? (nearbyRows[0] as any).nearby_count : 0;
    ...
    return sendSuccess(res, {
      ...
      search_scope: 'Vector similarity analysis using TiDB cosine distance'
    });

  } catch (error) {
    console.error('Similar reports error:', error);
    return sendError(res, 500, 'Internal server error', 'INTERNAL_ERROR', error);
  } finally {
    if (connection) {
      try {
